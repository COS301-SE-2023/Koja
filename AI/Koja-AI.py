...
def store_user_recommendations(user_id, user_recommendations):
    ...
    user_recommendations_json = json.dumps(user_recommendations)
...
def store_user_categories(user_id, event_names_per_category):
    ...
    event_names_per_category_json = json.dumps(event_names_per_category)

    response = table.put_item(
        Item={"user": user_id, "categories": event_names_per_category_json}
    )
...
def get_user_categories(user_id):
    ...
    if "Items" in response and len(response["Items"]) > 0:
        event_names_per_category = {item['categories']: json.loads(item['categories']) for item in response['Items']}
    else:
        event_names_per_category = {}

    return event_names_per_category
